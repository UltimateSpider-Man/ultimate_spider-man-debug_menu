...
#include "src/consolecmds.h"
#include "src/consolevars.h"
...
static void *HookVTableFunction2(void *pVTable, void *fnHookFunc, int nOffset) {
...
typedef int (__stdcall* GetDeviceState_ptr)(IDirectInputDevice8*, DWORD, LPVOID);
GetDeviceState_ptr GetDeviceStateOriginal2 = nullptr;
...
void GetDeviceStateHandleControllerInput(LPVOID lpvData) {
	...
	read_and_update_controller_key_button(joy, 11, MENU_TOGGLE);
        read_and_update_controller_key_button(joy, 10, MENU_R3);
...
#include "src/sound_instance_id.h"
...
void menu_setup(int game_state, int keyboard)
{
    ...
    if (is_menu_key_pressed(MENU_TOGGLE, keyboard) && (game_state == 6 || game_state == 7)) {
        ...
        if (!debug_enabled && game_state == 6) {
            ...
            static string_hash fx_ps_accept { "FE_PS_ACCEPT" };
            [[maybe_unused]] sound_instance_id v5 = sub_60B960(fx_ps_accept, 1.0, 1.0);
...
HRESULT __stdcall GetDeviceStateHook2(IDirectInputDevice8* self, DWORD cbData, LPVOID lpvData) {

	HRESULT res = GetDeviceStateOriginal2(self, cbData, lpvData);
...
GetDeviceData_ptr GetDeviceDataOriginal2 = nullptr;

HRESULT __stdcall GetDeviceDataHook2(IDirectInputDevice8* self, DWORD cbObjectData, LPDIDEVICEOBJECTDATA rgdod, LPDWORD pdwInOut, DWORD dwFlags) {

	HRESULT res = GetDeviceDataOriginal2(self, cbObjectData, rgdod, pdwInOut, dwFlags);
...
IDirectInput8CreateDevice_ptr createDeviceOriginal2 = nullptr;
...
HRESULT __stdcall IDirectInput8CreateDeviceHook(IDirectInput8W* self, const GUID* guid, LPDIRECTINPUTDEVICE8W* device, LPUNKNOWN unk)
{
    ...
    if (GetDeviceStateOriginal2 == nullptr) {
        GetDeviceStateOriginal2 = (GetDeviceState_ptr)
            HookVTableFunction2((void*)*device, (void*)GetDeviceStateHook, 9);
    }

    if (GetDeviceDataOriginal2 == nullptr) {
        GetDeviceDataOriginal2 = (GetDeviceData_ptr)HookVTableFunction2((void*)*device,
            (void*)GetDeviceDataHook2,
            10);
    }
...
HRESULT  __stdcall IDirectInput8CreateDeviceHook2(IDirectInput8W* self, const GUID* guid, LPDIRECTINPUTDEVICE8W* device, LPUNKNOWN unk) {
	...
	HRESULT res = createDeviceOriginal2(self, guid, device, unk);
	...
	if (!GetDeviceStateOriginal2) {
		GetDeviceStateOriginal2 = vtbl[9];
		vtbl[9] = GetDeviceStateHook2;
	}

	if (!GetDeviceDataOriginal2) {
		GetDeviceDataOriginal2 = vtbl[10];
		vtbl[10] = GetDeviceDataHook2;
	}
#else
    ...
    if (GetDeviceStateOriginal2 == nullptr) {
        GetDeviceStateOriginal2 = (GetDeviceState_ptr)
            HookVTableFunction2((void*)*device, (void*)GetDeviceStateHook2, 9);
    }
...
typedef HRESULT(__stdcall* DirectInput8Create2_ptr)(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID* ppvOut, LPUNKNOWN punkOuter);
HRESULT __stdcall HookDirectInput8Create2(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID* ppvOut, LPUNKNOWN punkOuter)
{
	DirectInput8Create2_ptr caller = (decltype(caller)) *(void**)0x00987944;
	...
	if (!createDeviceOriginal2) {
		createDeviceOriginal2 = vtbl[3];
		vtbl[3] = IDirectInput8CreateDeviceHook2;
	}
#else
    if (createDeviceOriginal2 == nullptr) {
        createDeviceOriginal2 = (IDirectInput8CreateDevice_ptr)
            HookVTableFunction2((void *) iDir, (void *) IDirectInput8CreateDeviceHook2, 3);
    }
#endif
...
HRESULT __stdcall HookDirectInput8Create(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID* ppvOut, LPUNKNOWN punkOuter)
{
    ...
    if (createDeviceOriginal == nullptr) {
        createDeviceOriginal = (IDirectInput8CreateDevice_ptr)
            HookVTableFunction2((void*)iDir, (void*)IDirectInput8CreateDeviceHook, 3);
    }

    return res;
}

DWORD hookDirectInputAddress2 = (DWORD) HookDirectInput8Create2;
...
uint8_t __fastcall os_developer_options2(BYTE *self, void *edx, int flag) {
...
void install_inifile(os_developer_options* a2)
{
    a2->m_flags;
    a2->m_ints;
    a2->m_strings;
    ini_parser::parse("game.ini", a2);
...
void install_patches()
{
	...
	*ReadOrWrite_data = &ReadOrWrite;
	printf("Replaced ReadOrWrite %08X -> %08X\n", (DWORD)ReadOrWrite_orig, (DWORD)&ReadOrWrite);
        ...
        DWORD hookDirectInputAddress = (DWORD)HookDirectInput8Create2;
	...
	*/
	...
	//HookFunc(0x005B87E0, os_developer_options2, 1, "Hooking os_developer_options::get_flag");
...
void stop_movie_handler(debug_menu_entry* entry)
{
...
void populate_script_menu(debug_menu_entry* entry)
{

            entry->set_game_flags_handler(stop_movie_handler);
        }
    

void populate_script_menu2(debug_menu_entry* entry)
        {
...
void create_script_menu_2(debug_menu* parent)
{
    auto* script_menu = create_menu("DEMO USM Trailer");
    auto* v2 = create_menu_entry(script_menu);
    v2->entry_type = UNDEFINED;
    v2->set_game_flags_handler(populate_script_menu);
    v2->set_game_flags_handler(populate_script_menu2);
    parent->add_entry(v2);
}
...
BOOL WINAPI DllMain(HINSTANCE hInstDll, DWORD fdwReason, LPVOID reserverd)
{
	...
	if (fdwReason == DLL_PROCESS_ATTACH) {
		...
		set_rdata_writeable();
		install_patches();
                os_developer_options* a2 = os_developer_options::instance();
                install_inifile(a2);
